...
### **画像生成開発支援プロンプト**
```
NIHONGO-AIプロジェクトの画像生成システムを{要求内容}したい。

【現在の画像生成システム】
- サポートサービス: DALL·E 3（デフォルト）、Unsplash
- APIキー管理: config.pyのget_api_key()で統一
- 画像サイズ制約: DALL·E 3は1024x1024等のみサポート
- コマンド例: python image_generator.py --article-id 006 --service dalle

【実装したい変更】
{具体的な変更内容}

【制約条件】
- APIキーはconfig.pyのget_api_key()で取得
- DALL·E 3のサイズ制約に対応
- 既存のcaptions.json構造を維持
- エラーハンドリング強化
...
### **記事自動生成プロンプト**
```
NIHONGO-AIの記事自動生成システムで{テーマ}に関する記事を作成したい。

【システム仕様】
- Claude API: 記事生成・翻訳
- DALL·E 3: 画像生成（デフォルト）
- 4言語対応: ja/en/zh-TW/zh-CN
- 自動メタデータ生成
...
【実行コマンド】
python main.py ideas --theme "{テーマ}"
python main.py full {記事ID}

【期待する出力】
完全な記事（マークダウン×4言語 + 画像 + メタデータ）
```